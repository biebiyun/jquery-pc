...
*/
    ...
    // 把时间选择框放到页面中
    $(0 < $('#appendParent').length ? '#appendParent' : '#rl').append(wrapper[this.mOpts.theme].join(''));
...
pickerDateRange.prototype.init = function(isCompare) {
    ...
    // 日历结束时间
    this.calendar_endDate = new Date(endDate.getFullYear(), endDate.getMonth() + 2, 0);
    ...
    $('#' + this.preMonth).bind('click', function() {
        __method.calendar_endDate.setMonth(__method.calendar_endDate.getMonth() -2, 1);
		...
		$('.ta_calendar table').eq(0).find('caption').css({"text-align":"left"});
			$('.ta_calendar table').eq(1).find('caption').css({"text-align":"right"});
        return false;
    });
    // 下一个月
    $('#' + this.nextMonth).bind('click', function() {
        __method.calendar_endDate.setMonth(__method.calendar_endDate.getMonth(), 1);
        __method.mOpts.endDate = __method.date2ymd(__method.calendar_endDate).join('-');
		__method.init(isCompare);
		//如果是单月选择的时候，要控制input输入框 added by johnnyzheng 2011-12-19
		if(1 == __method.mOpts.calendars){
...
pickerDateRange.prototype.selectDate = function (ymd) {
    ...
    if (this.startDateId == this.dateInput) {
        ...
        // 更改对应输入框的值
        $('#intime').val(ymdFormat);
        $('#outtime').val("");
        $('#intime2').html(ymdFormat);
        $('#outtime2,#days,#days2').html("");
        $('#' + this.endDateId).val("");
        ...
        // 如果开始时间未选
        if ('' == $('#' + this.startDateId).val()||ymdFormat == $('#' + this.startDateId).val()) {
            this.dateInput = this.startDateId;
            this.selectDate(ymd);
            return false;
        }
        ...
        $('#' + this.dateInput).val(ymdFormat);
        $('#intime').val($('#' + this.startDateId).val());
        $('#outtime').val($('#' + this.endDateId).val());
        $('#intime2').html($('#' + this.startDateId).val());
        $('#outtime2').html($('#' + this.endDateId).val());
        var ds = (Date.parse($('#outtime').val()) - Date.parse($('#intime').val())) / 1000 / 60 / 60 / 24;
        var dsa = $('#intime').val().split("-");
        var ds1 = new Date(dsa[1] + '-' + dsa[2] + '-' + dsa[0]); //转换为12-18-2002格式  
        var dsa = $('#outtime').val().split("-");
        var ds2 = new Date(dsa[1] + '-' + dsa[2] + '-' + dsa[0]);
        var dss = parseInt(Math.abs(ds1 - ds2) / 1000 / 60 / 60 / 24); //把相差的毫秒数转换为天数  
        if (isNaN(ds)) {
            ds = dss;
        }
        $('#days').html(ds);
...
pickerDateRange.prototype.fillDate = function(year, month, index) {
    ...
    w = dateEnd.getDay();
    // 计算应该结束的日期
	if(dateEnd.getDate() + 6 - w>35){
    	dateEnd.setDate(dateEnd.getDate() + 6 - w);
	}else{
		dateEnd.setDate(dateEnd.getDate() + 13 - w);
	}
	...
	if(isTaTheme){
		...
		if(!this.mOpts.magicSelect){
			...
			if(index + 1 == this.mOpts.calendars) {	
				if(this.calendar_endDate.getFullYear()==yToday){
					if(this.calendar_endDate.getMonth()-1>mToday){
						$(td).append('<a href="javascript:void(0);" id="' + this.preMonth + '"><i class="i_pre"></i></a>');
					}	
				}else if(this.calendar_endDate.getFullYear()>yToday){
					$(td).append('<a href="javascript:void(0);" id="' + this.preMonth + '"><i class="i_pre"></i></a>');
				}
    ...
    for(var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
        if(d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
            ...
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '1';
        } else if((this.mOpts.stopToday == true && d.getTime() < today.getTime()-86400000) || d.getTime() < __method.mOpts.minValidDate * 1000 || ('' !== __method.mOpts.maxValidDate && d.getTime() > __method.mOpts.maxValidDate * 1000)) { // 当前时间之后的日期，或者开启统计之前的日期
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '2';
        } else { // 当前月日期
            deviation = '0';
