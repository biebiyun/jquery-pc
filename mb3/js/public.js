...
$(function(){
	...
	$('.langChoose ul li input').on('click',function(){ 
		var language1=$(this).attr('data-id');
		$.cookie("lang", language1, { path: '/' }); 
		window.location.reload();
	})
	$('.head_choose ul li input').on('click',function(){ 
	...
	if ($.cookie("lang") == null || $.cookie("lang") == "") {
	    if ($(".langChoose ul li input[data-title=" + lang + "]").length > 0) {
	    	$("#langShow").val($(".langChoose ul li input[data-title=" + lang + "]").val());
	        $("#index_roomnum").val($(".langChoose ul li input[data-title=" + lang + "]").val());
	        $.cookie("lang", $(".langChoose ul li input[data-title=" + lang + "]").attr("data-id"), { path: '/' });
	    } else {
	    	$("#langShow").val($(".langChoose ul li input").eq(0).val());
	        $("#index_roomnum").val($(".langChoose ul li input").eq(0).val())
	        $.cookie("lang", $(".langChoose ul li input").eq(0).attr("data-id"), { path: '/' });
	    }
	} else {
		$("#langShow").val($(".langChoose ul li input[data-id=" + $.cookie("lang") + "]").val());
	    $("#index_roomnum").val($(".langChoose ul li input[data-id=" + $.cookie("lang") + "]").val()); 
	}
	langpath = './lang/' + $(".langChoose ul li input[data-id=" + $.cookie("lang") + "]").attr("data-title") + '.xml';
//	TranslateElementsAsy(document, 'tt', 'innerHTML', langpath);
	...
	TranslateElementsAsy(document, 'td', 'innerHTML', langpath);
	TranslateElementsAsy(document, 'option', 'text', langpath);
	TranslateElementsAsy(document, 'i', 'innerHTML', langpath);
})
...
// 时间转化
var timeConvert=function(timeC){
	var newTime
	newTime = timeC.split("/"); 
	if($.cookie("lang")=="1"){
...
var timeConvert2=function(timeC){
	var newTime
	newTime = timeC.split("-"); 
	if($.cookie("lang")=="1"){
		newTime=newTime[0]+'年'+newTime[1]+'月'+newTime[2]+'日';
...
function paging1(pagjson, index, pagenub) {
	$(index).unbind();
    var showpagenub = 5;
    var l = $(pagjson).length;
    $(index).empty();
    if (l > pagenub) {
        $(index).show();
        $(index).append('<div class="pag_item"><span class="first">'+jstag("xmlmallb20")+'</span><span class="prv">'+jstag("xmlmallb22")+'</span><span class="leftmore">...</span><ul class="clearfix"></ul><span class="rightmore">...</span><span class="next">'+jstag("xmlmallb23")+'</span><span class="last">'+jstag("xmlmallb21")+'</span></div>');
